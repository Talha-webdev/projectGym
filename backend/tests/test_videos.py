...
async def test_create_video_as_admin(async_client: AsyncClient, override_get_db, mock_db, admin_headers):
    ...
    from tests.conftest import MockVideo
    video_mock.scalar_one_or_none.return_value = MockVideo()
    mock_db.execute.side_effect = [admin_mock, slug_mock, video_mock]
    payload = {
        "title": "New Video",
        "slug": "new-video",
    ...
    fake_resp = VideoResponse(
        ...
        cloudinary_url=payload["cloudinary_url"], view_count=0,
